...
# Retry configuration for free-tier rate limits (e.g. deepseek:free on OpenRouter)
_RATE_LIMIT_MAX_RETRIES = 8          # max attempts before giving up
_RATE_LIMIT_BASE_DELAY  = 10         # seconds — first wait
_RATE_LIMIT_MAX_DELAY   = 120        # seconds — cap on exponential backoff
        ...
        for attempt in range(1, _RATE_LIMIT_MAX_RETRIES + 1):
            try:
                ...
                if attempt == _RATE_LIMIT_MAX_RETRIES:
                    raise
                # Respect the server's Retry-After hint when available,
                ...
                if retry_after:
                    ...
                    base = _RATE_LIMIT_BASE_DELAY * (2 ** (attempt - 1))
                    wait = min(base, _RATE_LIMIT_MAX_DELAY) + random.uniform(0, 5)
                print(
                    f"\n[Rate limit] attempt {attempt}/{_RATE_LIMIT_MAX_RETRIES} — "
