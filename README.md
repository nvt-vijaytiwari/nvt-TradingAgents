...
# Codex OAuth does not need an API key; it reads ~/.codex/auth.json
```
...
Run every Dhan CSV through the Codex-backed pipeline:

```bash
python run_all_stocks.py --date 2026-05-29
```

Useful options:
- `--limit N` to test on a smaller subset
- `--report-dir PATH` to change where the committed batch artifacts are written
- `--checkpoint` is accepted but ignored for Codex batch runs until checkpoint metadata is fixed
...
We built TradingAgents with LangGraph to ensure flexibility and modularity. The framework supports multiple LLM providers: OpenAI, Google, Anthropic, xAI, DeepSeek, Qwen (Alibaba DashScope), GLM (Zhipu), OpenRouter, Ollama for local models, Azure OpenAI for enterprise, and Codex OAuth for keyless ChatGPT-backed runs.
